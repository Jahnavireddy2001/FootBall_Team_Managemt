...
<table >
   <tr>
    ...
    $sql = "SELECT * FROM players WHERE club_id=2";
$result = $con->query($sql);

if ($result->num_rows > 0) {

    while($row = $result->fetch_assoc()) {
        echo "<tr><td>" . $row["name"] . "</td><td>" . $row["nationality"] . "</td><td>" . $row["dob"] . "</td><td>" . $row["height"] . "</td><td>" . $row["jersy_no"] . "</td><td>" . $row["position"] . "</td></tr>";
